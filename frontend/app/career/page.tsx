...
export default function CareersPage() {
  ...
  const handleFileChange = (event, role) => {
    const file = event.target.files[0];
    if (file && file.type === "application/pdf") {
      alert(`Uploaded CV for ${role}: ${file.name}`);
          ...
          >
            ...
            <div className="p-2.5">
              ...
              <input
                ...
                className="hidden"
                ref={(el) => (fileInputRefs.current[career.id] = el)}
                onChange={(e) => handleFileChange(e, career.Role)}
              />

              <button
                onClick={() => handleUploadClick(career.id)}
